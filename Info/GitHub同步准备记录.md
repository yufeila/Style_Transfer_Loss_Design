...
已完成远程资产盘点、轻量文件准备和大文件清单。提交内容仅包含代码、配置、Info/TODO、核心结果图、SAM mask、metrics 和恢复说明；大文件不进普通 Git。
...
- 已新增 `.gitattributes`，为 `.pth`、模型目录、原始数据和归档文件预留 LFS 规则。
- 在安装 Git LFS 前，不应把 SAM 权重、原始数据或大型结果直接加入普通 Git。
...
rsync -az gpu-server:/data1/yyf/Style_Transfer_Loss_Design/data/ /data1/yyf/Style_Transfer_Loss_Design/data/
